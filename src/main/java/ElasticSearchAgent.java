...
public class ElasticSearchAgent {
    private static final String INDEX ="rss_store";
    ...
    public void addNews(List<News> news){
        for (News news_ : news) {
            ...
            IndexRequest request = new IndexRequest(INDEX, "item", newsHash);
            String source = gson.toJson(news_);
    ...
    public List<News> getSimilarNews(News news, int limit) throws IOException {
        ...
        MoreLikeThisQueryBuilder.Item newsItem = new MoreLikeThisQueryBuilder.Item(INDEX, "item", newsHash);
